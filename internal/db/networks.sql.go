...
const getNetworkByAgentID = `-- name: GetNetworkByAgentID :one
SELECT id, name, cidr, site, agent_id, metadata, created_at, updated_at
FROM networks
WHERE agent_id = ?
LIMIT 1
...
func (q *Queries) GetNetworkByAgentID(ctx context.Context, agentID *string) (Network, error) {
	row := q.db.QueryRowContext(ctx, getNetworkByAgentID, agentID)
	var i Network
	err := row.Scan(
		&i.ID,
		&i.Name,
