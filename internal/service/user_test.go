...
func TestPasswordPolicy_Configurable(t *testing.T) {
	...
	if err := validatePassword(relaxed, "2022-02-22", "admin"); err != nil {
	...
	if err := validatePassword(config.PasswordPolicyConfig{MinLength: 11}, "2022-02-22", "admin"); err == nil {
		t.Error("min_length must still be enforced when relaxed otherwise")
	}
	if err := validatePassword(DefaultPasswordPolicy(), "2022-02-22", "admin"); err == nil {
