...
// TestApplyDeviceBridge_RandomizedMAC_DowngradesIdentity is the headline
// correctness test for issue #118 Phase 1: a MAC with the locally-administered
// (LAA) bit set (e.g. 02:.. — an iOS/Android/Windows privacy-randomized MAC)
// must NOT anchor cross-network identity. A device that ROAMS between two
// networks carrying the SAME randomized MAC must be registered as TWO separate
// rows — the opposite of the stable-MAC roaming case above. Without this
// downgrade, each randomized MAC would be globally deduped and the registry
// would merge devices that happen to share a random byte pattern; worse, a
// device that rotates its random MAC on every scan would still wrongly dedup
// against a prior row that lost the race. The MAC is still stored for display.
//
// This test uses TWO networks to exercise the cross-network path: it creates a
// second network and applies the report against that network's id, then asserts
// a second row was created (vs. the single row the stable-MAC case produces).
func TestApplyDeviceBridge_RandomizedMAC_DowngradesIdentity(t *testing.T) {
	rn, queries, conn := setupChangeDetectDB(t)
	ctx := context.Background()

	// First sighting on the default network with a randomized MAC (LAA bit set).
	const randMAC = "02:11:22:33:44:55" // 02 → low nibble 2 & 0x2 = LAA
	_, _ = rn.applyDeviceBridge(ctx,
		reportFor("192.168.63.10", "phone", "apple", randMAC), rn.networkID, "")
	require.Equal(t, 1, countDevices(t, conn), "first sighting creates one row")

	// Create a SECOND network so we can scan the same randomized MAC on a
	// different network — the cross-network global-MAC-merge path that must NOT
	// fire for a randomized MAC.
	net2, err := queries.CreateNetwork(ctx, db.CreateNetworkParams{Name: "test-net-2"})
	require.NoError(t, err)
	net2ID := sql.NullInt64{Int64: net2.ID, Valid: true}

	// Same randomized MAC, different network + ip. A STABLE mac here would match
	// the existing row globally (one asset, roaming). A RANDOMIZED mac must NOT —
	// identity falls back to (ip, network_id), creating a second row.
	_, _ = rn.applyDeviceBridge(ctx,
		reportFor("192.168.62.20", "phone", "apple", randMAC), net2ID, "")

	require.Equal(t, 2, countDevices(t, conn),
		"randomized MAC must NOT globally dedup across networks — two rows expected")

	// The MAC is still stored on BOTH rows (identity downgraded, MAC preserved).
	rows := deviceRowsByMAC(t, conn, randMAC)
	require.Len(t, rows, 2, "both rows carry the randomized MAC for display")
	for _, r := range rows {
		require.Equal(t, randMAC, r.MAC, "MAC stored as an observed attribute even when not used for identity")
	}
}

// TestApplyDeviceBridge_RandomizedMAC_StableAcrossRescans verifies that when a
// randomized-MAC device is RESCANNED at the SAME (ip, network), it correctly
// UPDATES the existing row rather than creating a duplicate. This is the
// correctness counterpart: downgrading to (ip, network_id) identity must still
// keep a single asset per (ip, network) across rescans.
func TestApplyDeviceBridge_RandomizedMAC_StableAcrossRescans(t *testing.T) {
	rn, _, conn := setupChangeDetectDB(t)
	ctx := context.Background()
	const randMAC = "1a:bb:cc:dd:ee:ff" // 1a → low nibble a (1010) & 0x2 = LAA

	_, _ = rn.applyDeviceBridge(ctx,
		reportFor("192.168.63.30", "phone", "apple", randMAC), rn.networkID, "")
	// Rescan the SAME ip+network with the SAME randomized MAC.
	_, _ = rn.applyDeviceBridge(ctx,
		reportFor("192.168.63.30", "phone", "apple", randMAC), rn.networkID, "")

	require.Equal(t, 1, countDevices(t, conn),
		"same (ip, network) randomized-MAC rescan must update, not duplicate")
}

// deviceRowsByMAC returns all device rows carrying the given mac_address, used
// to assert identity-downgrade tests don't drop the MAC from storage.
func deviceRowsByMAC(t *testing.T, conn *sql.DB, mac string) []deviceRow {
	t.Helper()
	rows, err := conn.Query(
		`SELECT id, ip_address, mac_address, status, brand, name FROM devices WHERE mac_address = ?`, mac)
	require.NoError(t, err)
	defer rows.Close()
	var out []deviceRow
	for rows.Next() {
		var r deviceRow
		require.NoError(t, rows.Scan(&r.ID, &r.IP, &r.MAC, &r.Status, &r.Brand, &r.Name))
		out = append(out, r)
	}
	return out
}
