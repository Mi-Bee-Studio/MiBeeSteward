...
- **分布式发现**：在远程局域网部署轻量采集器，跨网络发现设备。采集器通过拉取模型 HTTPS 上报到中心，bearer 令牌认证、断线补报、MAC 优先设备身份（同设备跨网络保持单一资产；随机/隐私 MAC 会被识别并降级为按网络身份，避免产生幽灵设备）。[分布式指南](docs/zh/distributed-guide.md)
