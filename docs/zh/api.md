...
所有端点共同遵循的约定。本节的机器可读形式是 [`docs/openapi.yaml`](../openapi.yaml)（由 CI 奇偶校验测试保证与实现同步）；这里是文字契约。
...
`limit` / `offset` 查询参数，策略全局一致（`ParsePagination` 统一实施）：

- 缺省（或 `0`）→ 端点默认值
- **非法或负值 → 400**
- **超过端点上限 → 钳制到上限**（绝不是报错；通过回显的 `limit` 可检测）

| 端点族 | 默认 | 上限 |
|---|---|---|
| 设备、用户、文档、扫描结果/运行/任务、拨测目标、设备系统、通知日志 | 20 | 100 |
| 变更、agent 命令（all）、网络授权 | 50 | 200 |
| 心跳结果/历史、审计日志 | 50 | 500 |
| 设备配置版本 | 100 | 1000 |
| SNMP 凭据 | 100 | 1000 |
| SSH 凭据 | 50 | 200 |
...
所有时间戳是 RFC 3339 字符串。存储侧派生字段携带服务器时区偏移；集成方应使用完整 RFC 3339 解析器（而非裸日期解析器）。
...
**响应**：`201 Created` 带 ProbeTargetResponse（含 `last_run_at`/`last_status`/`last_latency_ms`/`last_error` 去规范化最新结果--只反映中心轨道，空 = 尚未探测；另含 `vantage_latest`--各视角轨道的最新一条结果（含 agent 轨道），尚无历史时省略）。
...
列表端点使用 `limit` 与 `offset` 查询参数，遵循 [API 契约](#api-契约274) 策略：缺省/0 → 端点默认值，非法或负值 → 400，超上限 → 钳制（经回显的 `limit` 可检测）。各端点的默认值/上限见契约章节的表格。

响应为 `{ "<items>": [...], "total": <int>, "limit": <int>, "offset": <int> }` 信封，`total` 为过滤后的总数，limit/offset 回显实际生效的分页。
