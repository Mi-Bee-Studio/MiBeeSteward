...
本文档是 MiBee 指纹库的**规范性文件**（normative specification），定义了语言无关、数据驱动的设备识别规则格式。

**谁需要读它：**

- **规则文件作者** — 编写 YAML 指纹规则，需要理解规则格式、匹配操作、置信度模型和提取操作
- **适配器实现者** — 用 Go/Zig/Rust 等语言实现指纹库加载器，需要遵循本文档定义的契约以确保跨语言一致的分类结果

**核心概念速览：**

| 概念 | 说明 |
|------|------|
| 语料库（Corpus） | YAML 规则文件集合，CC-BY-SA 4.0 许可 |
| 规则格式 | `version: 1` + `rules:` 列表，每条规则包含 id、match、service、confidence、extract 等字段 |
| 匹配语义 | 叶子操作（prefix/contains/regex 等）+ 组合操作（compound/or），支持任意嵌套 |
| 置信度融合 | 默认 `fused = 1 - (1 - evidence.conf) * (1 - rule.conf)`；`literal_confidence: true` 时直接使用规则置信度 |
| 许可边界 | CC-BY-SA 4.0 适用于规则语料库；IEEE OUI/IANA PEN 为事实注册表，仅引用不纳入；Recog 规则保留 Apache-2.0 出处标注；nmap-service-probes（NPSL）不导入 |
