...
- **Distributed Discovery**: Deploy lightweight agents on remote LANs to discover devices across networks. Agents report to a central hub via pull-model HTTPS with bearer-token auth, disconnect recovery, and MAC-primary device identity (same device stays one asset across networks; randomized/privacy MACs are detected and downgraded to per-network identity so they don't spawn ghost devices). [Distributed Guide](docs/en/distributed-guide.md)
