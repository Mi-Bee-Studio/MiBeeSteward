...
<script lang="ts">
	...
	const allNavItems: { path: string; label: string; icon: Component; adminOnly?: boolean }[] = [
		...
		{ path: '/topology', label: m['navigation.Topology'](), icon: Share2 },
		{ path: '/discovery', label: m['navigation.Discovery'](), icon: Radar },
		{ path: '/networks', label: m['navigation.Networks'](), icon: Network, adminOnly: true },
		{ path: '/agents', label: m['navigation.Agents'](), icon: Bot, adminOnly: true },
