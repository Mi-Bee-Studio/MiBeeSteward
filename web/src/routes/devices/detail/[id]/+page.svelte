		...
		{#if hasDiscoveryData}
			<div class="scan-info-panel mt-4">
				...
				<div class="scan-info-grid">
					...
					{#if sa.cpu_count || sa.cpu_model}<div class="scan-info-field"><span class="scan-info-label">{m['scanfields.CPU']()}</span><span class="scan-info-value">{sa.cpu_model ? sa.cpu_model : m['scanfields.CPU Value']({ count: sa.cpu_count ?? 0 })}{#if sa.cpu_model && sa.cpu_count} {m['scanfields.vCPU Count']({ count: sa.cpu_count ?? 0 })}{/if}</span></div>{/if}
